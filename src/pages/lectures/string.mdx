...
高中只要知道怎麼在 $O(N \log N)$ 時間就好（基本上線性解賽中根本寫不完）
...
for (int i = 0; i < 20; i++) {
    ...
    for (int j = 0; j < n; j++) {
        v[j].first = ord[i];
        if (j + (1<<i) < n) v[j].second = ord[j+(1<<i)];
        else v[j].second = -1;
    }
    sort(sa.begin(), sa.end(), [&](int a, int b) {
        return v[a] < v[b];
...
// sa is done!
```
應該容易看出複雜度是 $O(N \log ^ 2 N)$

### 用 radix sort 替代 sort
```cpp
int n;
int rnk[mxn];
...
for (int i = 1; i = n; i <<= 1) bucket_sort(i);
```

### LCP
```cpp
for (int i = 0; i < n; i++) pos[rnk[i]] = i;
for(int i = 0;i<n;i++){
    int pi = rnk[i];
    if(pi == 0){
        k = max(k-1,0);
...
### 各種用途
- 找兩個 substring 的 LCP：問區間最小值
- 問多少相異 substring: 枚舉開頭之後簡單排容

### 例題
- [CSES Repeating Substring](https://cses.fi/problemset/task/2106)
- [CSES Substring Order](https://cses.fi/problemset/task/2108/)
- [CSES Substring Distribution](https://cses.fi/problemset/task/2108/)
- [TIOJ 2155](https://tioj.ck.tp.edu.tw/problems/2155)：這題常數很緊，所以可能要用更強的 SA 去壓或者是抄別人的模板

### 線性 SA
主流有兩種： SAIS 跟 DC3 ，而目前大學競賽有以 SAIS 為主，因為比 DC3 快非常多。
