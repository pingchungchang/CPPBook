...
int N, M;
const int mxn = 110;
ll arr[mxn][mxn]; // arr[i][0] ~ arr[i][N-1]; arr[i][N] = x_i
...
vector<int> gauss_elim() {
    for (int i = 0; i < N; i++) {
        int p = i;
        while (p < N && arr[p][i] == 0) p ++;
        for (int j = 0; j < N; j++) swap(arr[i][j], arr[p][j]);
        ll coef = inv(arr[i][i]);
        for (int j = 0; j <= N; j++) arr[i][j] = mul(arr[i][j], coef);
        for (int j = i+1; j < N; j++) {
            ll c = arr[j][i];
            c = mub(0, c);
            for (int k = 0; k <= N; k++) {
                arr[j][k] = mad(arr[j][k], mul(c, arr[i][k]));
            }
        }
    }
    for (int i = N-1; i >= 0; i--) {
    ...
    vector<int> ans(N);
    for (int i = 0; i < N; i++) ans[i] = arr[i][N];
    return ans;
}
```
